...
export const AnimatedTeacher = forwardRef<TeacherRefHandle, CharacterProps & { avatarStyle?: 'standard' | 'nft' }>(
  (
    {
    ...
    const [currentExpression, setCurrentExpression] = useState<ExpressionType | undefined>(initialPropsExpression);
    const [currentGesture, setCurrentGesture] = useState<GestureType | undefined>(initialPropsGesture);
    const [currentPosition, setCurrentPosition] = useState<PositionPreset | { x: number; y: number }>(initialPropsPosition);
    const [currentGaze, setCurrentGaze] = useState<GazeTarget | undefined>(initialPropsGaze);
    const [currentPointTarget, setCurrentPointTarget] = useState<PointTarget | undefined>(initialPropsPointTarget);
    ...
    useImperativeHandle(
      ref,
      () => ({
        play: (newState: CharacterState | 'idle' | 'explain' | 'thinking' | 'question' | 'listen' | 'correct' | 'celebrate' | 'noPeek', durationMs?: number) => {
          let resolvedState = newState as CharacterState;
          if (newState === 'explain') resolvedState = 'explaining';
          if (newState === 'listen') resolvedState = 'listening';
    ...
    const isPresetPosition = typeof currentPosition === 'string' && currentPosition in POSITION_STYLES;
    const positionClass = isPresetPosition ? POSITION_STYLES[currentPosition as PositionPreset] : '';
    const customPositionStyle =
      typeof currentPosition === 'object' ? { left: `${currentPosition.x}px`, top: `${currentPosition.y}px` } : {};
