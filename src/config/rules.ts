...
/** Scores a student recitation against the chapter's exam keywords. */
export function gradeRecitation(input: string, module: NcertModule): RecitationVerdict {
  const said = normalize(input).replace(/\s/g, "");
  const matched: string[] = [];
  const missing: string[] = [];

  for (const kw of module.examKeywords) {
    const k = normalize(kw).replace(/\s/g, "");
    if (k && said.includes(k)) matched.push(kw);
